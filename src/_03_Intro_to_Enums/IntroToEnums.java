...
public class IntroToEnums {
	...
	public static void main(String[] args) {
		...
		String s = JOptionPane.showInputDialog("Pick a state of matter");
		// 5. Iterate through the array and find what the user entered.
		// Hint: .name() or .toString
		for(StatesOfMatter m: arr) {
			if(s.equals(m.name())) {
		...
		StatesOfMatter a = arr[(int)Math.random()*arr.length];
		// 11. Print outs both of its temperatures.
		System.out.println("Cel: "+a.cel()+"; Fah: "+a.convertToFahrenheit(a.cel()));
		...
		switch(a) {
			case GAS:
				System.out.println("Flavored air");
				break;
			case LIQUID:
